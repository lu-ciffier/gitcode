 ...
 */
     ...
     * **/
	function start() {
		$.post("open_socket.action", {platform:"web"},function(data) {
			if ($.trim(data) != "") {
				//alert("ok");				
			}
		});
	...
	function menue(command) {
		...
		if (command == "singlecollect") {
			...
			$.post("command_socket.action", para, function(data) {
				//isworking = true;
			});
		...
		if (command == "allcollect") {		
			$.post("command_socket.action", {"data":"264153413030303030303030303030303030303030303030303030303030302A"}, 
					function(data) {	
			});
		...
		if (command == "allpuase") {
			$.post("command_socket.action", {"data":"264153543030303030303030303030303030303030303030303030303030302A"}, 
					function(data) {	
			});
			for ( var w in workingstate) {
